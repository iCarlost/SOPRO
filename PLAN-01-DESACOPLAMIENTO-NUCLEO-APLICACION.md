...
**N7-18a (andamiaje, listo para dictamen GO/NO-GO, sin integrar):** goldens legacy del "Catálogo de matrices" en el proyecto temporal `SOPRO.WinForms.Tests` (sin tocar producción; se elimina en N7-18e): `GeneradorExcelCatalogoMatricesTests` caracteriza el xlsx real vía snapshot semántico JSON (celdas/formatos/estructura/página: sintético y proyecto real ***REMOVED*** CD ***REMOVED***, 7 matrices; evidencia cruda de aritmética de las filas 22-23 de N0-TABLA almacenada en el archivo: 30.015, 30.01, 3.001; reales ***REMOVED*** y ***REMOVED***) y `GeneradorPdfCatalogoMatricesTests` caracteriza el PDF con hash del flujo **normalizado** (fechas/IDs fijos + parches byte a byte con alcance acotado: tags solo en `/BaseFont`/`/FontName`, fechas y UUIDs solo dentro de sus elementos XMP del paquete `xpacket`, fail-fast ante forma desconocida y test centinela de que el mismo patrón fuera de esos campos queda intacto) más manifiesto; el determinismo se valida primero con el test de "originales distintos → normalizados idénticos". Gates verdes: 6/6 Debug y Release, SOPRO.Tests 611/611, build sln Release 0 errores, `git diff --check` limpio.
