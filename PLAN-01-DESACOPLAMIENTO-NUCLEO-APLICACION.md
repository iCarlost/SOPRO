...
- Pendiente para PRs N4 posteriores (orden del plan): `ProjectLifecycleService` (expone `session.Context`), `ExternalMatrixImportService`/`ExternalInsumoImportService` (contexto externo), `MatrixConsolidationService`, `SchemaManager`; bloqueo de workspace (`.lock` + `FileShare.None`, decisión confirmada); prohibición de `Task.Run` con contexto (8 ocurrencias en `FormProgramaObra.*`/`FormProyecto.Formato.cs`); snapshots para cálculo en segundo plano; pruebas de hilo con contador de accesos concurrentes.
