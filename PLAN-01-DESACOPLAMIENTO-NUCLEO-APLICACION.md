...
**N7-17c integrado (PR #76, `d0980d9`):** `FinancingCalculationAdapter` en Sopro.Application consulta EF, prepara los escalares por período (CD/CI/estimación desde `ImporteProgramado`), mapea a `FinancingInput` y persiste el resultado: reemplazo de filas de flujo, mutación de la configuración y `FechaCalculo`. `FinanciamientoCalculationService.Calcular` delega en el adaptador; las guardas del legado (programa/periodos/base ≤ 0 → 0 sin tocar filas ni config) se preservan en el mismo orden y la base se comprueba antes de sumar el presupuesto. Los 8 goldens de N7-17a pasan contra el servicio delegado y se añadieron tests de delegación (recálculo con reemplazo de filas, guardas con contenido completo e IDs, horas preservadas en filas base). Pendiente (evaluación separada): centralizar la preparación CD/CI con absorción de residuo.
