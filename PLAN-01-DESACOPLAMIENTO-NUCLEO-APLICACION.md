...
**Estado:** N0 cerrado (PR #2, `508188a`); N1 cerrado (PR #3, `b9a4576`+`eaa2f67`); N2 cerrado; N3 (slice inicial de Application) cerrado; N4 cerrado (PRs N4-1..N4-5 integrados, Gate N4 operativo cerrado); N5 cerrado (PRs N5-20..N5-25 integrados, Gate N5 cerrado); N6 cerrado (PRs N6-1..N6-5 integrados, Gate N6 operativo cerrado); N7 cerrado GO (N7-1a..N7-1e integrados con evaluador puro de grafos, adaptador de snapshot y orden topológico canónico compartido por importación/propagación/recálculo global, más verificación de aciclicidad del grafo externo; N7-2 integrado: signos conservados en Explosión/Programación, fila 21 de N0-TABLA; N7-3 integrado: importes unitarios compartidos vía MatrixComponentCalculationService.ImportesUnitarios; N7-4 integrado: distribución proporcional con absorción de residuo compartida vía DistribuirImporteProporcional, eliminadas las tres copias; N7-5 integrado: mapeo BudgetPercentageInput → PricePercentageInput consolidado en BudgetPercentageInput.ToPricePercentage(); N7-6 integrado: caracterización de aritmética cruda de reportes APU, catálogos y presupuesto sin migrar código, filas 22-26 de N0-TABLA; N7-7 integrado: construcción única FromProyecto para inputs de porcentaje; N7-8 integrado: fábrica única CalculationEngineFactory.FromProyecto; N7-9 integrado: retirada de wrappers privados triviales; N7-10 integrado: centralización del cálculo de rendimiento de maquinaria; N7-11 integrado: mapper único para entidades importadas con diferencial campo a campo; N7-12 integrado (PR #65, `e2313ca`): cálculo de costo horario puro del motor independiente, delegado por Core, WinForms y reportes; N7-13 integrado (PR #66): caracterización y centralización del FSR; N7-14 integrado (PR #67, `eac4eba`): centralización de indirectos y utilidad; N7-15 integrado (PR #68, `e139454`): retirada de código muerto de explosión; N7-16a integrado (PR #69, `0d1cd67`): caracterización de calendario y red de actividades; N7-16b integrado (PR #70, `682e9f8`): calendario puro en SOPRO.Calculation; N7-16c integrado (PR #71, `6cc28f7`): red y ruta crítica puras en SOPRO.Calculation.); N7-16d integrado (PR #72, `f7dfdb9`): delegación de la red, ruta crítica y primitivas de calendario del programa de obra a los calculadores puros (`ActivityNetworkCalculator`/`WorkingCalendarCalculator`), con rechazo formal de ciclos y goldens de paridad sobre `ProgramacionCalculationService`; N7-16e integrado (PR #73, `88fd081`): delegación del predicado/conteo de día hábil de `ProgramacionDistributionService` y `ProgramacionLoadService` a `WorkingCalendarCalculator` vía `WorkingCalendarAdapter`, con goldens de excepciones de calendario (Inhabil/LaborableEspecial) sobre el split de la distribución uniforme y la duración de resúmenes.
**Estado N7 (6.2):** N7 CERRADO/GO conforme a [N7-MATRIZ-CIERRE.md](N7-MATRIZ-CIERRE.md): advertencias 0/0 Debug/Release, compilación/pruebas 656/656 Debug/Release, Gate N0 44/44, catálogo 40/40 y consumer package verde. Nota factual conservada: el baseline histórico 421/446 no es reproducible y el baseline final es 0/0; el cierre GO es una decisión explícita que acepta esa limitación de reproducibilidad histórica sin reclasificar 421/446 como advertencia vigente.
**Gate N7:** CERRADO/GO. Esto no cierra el Plan 01/ADR-001 ni el Plan 02 (WPF).

**N7-17a integrado (PR #74, `c7bc837`):** caracterización del flujo legacy de financiamiento (`FinanciamientoCalculationService`) mediante goldens calculados a mano — anticipo y amortización con caps, desfase de cobro, base "SobreCD" vs "Acumulable", modelo dual con saldos mixtos y diferencial de tasa real (TIIE 12% vs efectiva 15%), residuo de distribución, y guards (sin programa/periodos/base 0) — sin migrar código; sirve de oráculo para la centralización en Sopro.Calculation.
**N7-17b integrado (PR #75, `b67519a`):** calculador puro `FinancingCalculator` en Sopro.Calculation con entrada/resultado inmutables, base de acumulación como enum (`FinancingBaseCalculationMode`), política de precisión explícita (`FinancingPrecision`; legado 8/6/4/5) y tests puros contra los 8 goldens de N7-17a y bordes; sin delegar producción aún.
**N7-17c integrado (PR #76, `d0980d9`):** `FinancingCalculationAdapter` en Sopro.Application consulta EF, prepara los escalares por período (CD/CI/estimación desde `ImporteProgramado`), mapea a `FinancingInput` y persiste el resultado: reemplazo de filas de flujo, mutación de la configuración y `FechaCalculo`. `FinanciamientoCalculationService.Calcular` delega en el adaptador; las guardas del legado (programa/periodos/base ≤ 0 → 0 sin tocar filas ni config) se preservan en el mismo orden y la base se comprueba antes de sumar el presupuesto. Los 8 goldens de N7-17a pasan contra el servicio delegado y se añadieron tests de delegación (recálculo con reemplazo de filas, guardas con contenido completo e IDs, horas preservadas en filas base). La preparación CD/CI se centralizó en N7-17d.
**N7-18a integrado (PR #78, `c976477`):** goldens legacy del "Catálogo de matrices" en el proyecto temporal `SOPRO.WinForms.Tests` (sin tocar producción; se elimina en N7-18e): `GeneradorExcelCatalogoMatricesTests` caracteriza el xlsx real vía snapshot semántico JSON (celdas/formatos/estructura/página: sintético y proyecto real ***REMOVED*** CD ***REMOVED***, 7 matrices; evidencia cruda de aritmética de las filas 22-23 de N0-TABLA almacenada en el archivo: 30.015, 30.01, 3.001; reales ***REMOVED*** y ***REMOVED***) y `GeneradorPdfCatalogoMatricesTests` caracteriza el PDF con hash del flujo **normalizado** (fechas/IDs fijos + parches byte a byte con alcance acotado: tags solo en `/BaseFont`/`/FontName`, fechas y UUIDs solo dentro de sus elementos XMP del paquete `xpacket`, fail-fast ante forma desconocida y test centinela de que el mismo patrón fuera de esos campos queda intacto) más manifiesto; el contrato de determinismo es la igualdad de los normalizados (que los originales difieran se registra como diagnóstico). Gates verdes: 10/10 Debug y Release (6 goldens + 4 sentinel), SOPRO.Tests 611/611, build sln Release 0 errores, `git diff --check` limpio.
**N7-18b corregido tras dictamen NO-GO (rama `feat/n7-18b-modelo-neutral-catalogo`):** modelo neutral y caso de uso del catálogo de matrices en `SOPRO.Application` (NO en Calculation; un documento de reporte es un resultado de aplicación, las filas 22-23 son compatibilidad legacy explícitamente no canónica). `Models/Reporting/MatrixCatalog` expone el documento inmutable (`MatrixCatalogReportDocument` con Title/TitleStyle/ProjectName, zones de encabezado y pie, y `MatrixCatalogMatrix[]` con Kind/Key/Description/Unit/DirectCost (persistido)/TotalMo (crudo) y `MatrixCatalogComponent[]` con Kind/Prefix/Key/Description/Unit/Quantity/UnitCost/Amount), enums propios y escalares neutrales (fuente string, tamaño double, negrita/cursiva, color de texto hex, alineación propia); el color de fondo del título es política de renderer (Excel #33334C vs PDF #1F4E79; NO vive en el modelo — dictamen NO-GO); NO hay celdas, direcciones A1, rangos, anchos, constantes PDF, rutas de salida, entidades EF ni objetos UI. EF transitorio en Application vía `IProjectDbContextFactory` (patrón ListMaterials: contexto por operación, AsNoTracking, `CancellationToken` en cada I/O, proyección pura `MatrixCatalogSourceMapper` carga/proyección separables, nunca devuelve entidades). Arirmética pura en `MatrixCatalogReportModelBuilder`: totalMO crudo (MO no-%MO × cantidad + auxiliar cuadrilla × cantidad), %MO → importe = totalMO × cantidad y base mostrada = totalMO, importe = cantidad × costoUnitario, orden ordinal por clave (defensivo ante entrada barajada), total de matriz = CostoDirecto persistido; sin redondeo ni cálculo canónico. Tokens resueltos desde Execute con TimeProvider inyectado en el caso de uso (fecha_impresion; gate de reloj explícito PLAN-01:708) y formato invariante; {pagina}/{total_paginas} quedan al medio. Contratos inmutables por materialización defensiva (`ReadOnlyCollection<T>`), validación de nulos antes de guardas semánticas. 24 tests en `SOPRO.Tests/UseCases/Reporting/MatrixCatalogReportUseCasesTests.cs` (sin dependencia de WinForms): puente con N7-18a en el modelo (30.015 / 30.01 / 3.001 sintético; ***REMOVED*** y ***REMOVED*** reales; 7 matrices), orden barajado, ids duplicados/inexistentes (WHERE IN legacy), aislamiento por proyecto en Execute: igualdad estricta `m.ProyectoId == projectId` (sin escape por sesión maestra: `ProjectRef.Master` se rechaza antes de abrir la base con `AppErrorCode.Validation`, dictamen NO-GO) + test SQLite de dos proyectos con IDs de matriz reales, request con copia defensiva de `MatrixIds` envuelta con `Array.AsReadOnly` (un `int[]` crudo no se conserva: cast a `int[]`/`IList<T>` fallan y el setter por índice y `Add` lanzan — gate de inputs inmutables, dictamen NO-GO), inmutabilidad real, cultura y reloj explícitos, sin mutación de input, proyección con navegaciones nulas. NO compara celdas ni hashes (eso es N7-18c cuando `SOPRO.Reporting` renderice sobre los mismos 6 goldens). El documento conserva `ProjectName` (el PDF legacy lo imprime explícitamente; semántico — dictamen NO-GO). Gates: SOPRO.Tests Debug y Release 635/635 (631 + 4 nuevos), goldens N7-18a 10/10, build sln Release 0 errores, `git diff --check` limpio.

**N7-18c integrado (PR #80, `c8aeba8`):** renderers neutrales Excel y PDF del Catálogo de matrices en `SOPRO.Reporting`, que reciben el modelo neutral — sin dependencia de WinForms, EF ni generadores legacy. Plantilla libre para PDF (layout dinámico); fixture real (***REMOVED*** CD ***REMOVED***, 7 matrices) migrado con goldens preservados; paridad de contenido validada contra los snapshots de N7-18a.

**N7-18d integrado (PR #81, `9a8249f`):** swap de callers WinForms al pipeline Application→Reporting: `FormMatrices` delega vía `CatalogoMatricesExportService` (adaptador fino en WinForms, composición sincrónica caso de uso + renderer). El adaptador no toca el destino ante fallo (sin escritura parcial).

**N7-18e integrado por PR #82 (`69d9cfa`) (rama `feat/n7-18e-retirada-generadores-legacy`, base `origin/main` `9a8249f` como merge de PR #81 de N7-18d):** retirada de los 2 generadores legacy del Catálogo de matrices (`GeneradorExcelCatalogoMatrices`, `GeneradorPdfCatalogoMatrices`) y del proyecto temporal `SOPRO.WinForms.Tests` (5 fuentes + salida de `SOPRO.sln`; 8 archivos, 1437 eliminaciones), tras el swap N7-18d al pipeline nuevo (`BuildMatrixCatalogReport` + renderers de `SOPRO.Reporting`). Cero callers compilables (`new Generador*CatalogoMatrices` y `WinForms.Tests` ausentes en producción y solución; solo quedan menciones históricas en comentarios, N0-TABLA y metadata de goldens). Goldens y metadata histórica preservados en `SOPRO.Reporting.Tests/TestData/Goldens`. Evidencia Fase 3: focalizadas 7/7 renderers + 33/33 caso de uso, solución completa 655/655 Debug y 655/655 Release con 0 warnings/errores, `git diff --check` limpio; riesgos aceptados (oráculo legacy sustituido por goldens versionados, cobertura del servicio fino N7-18d cubierta por suites supervivientes). Gate GO.

**N7-17d integrado (PR #77, `3c118a5`):** el adaptador delega la **preparación numérica CD/CI** al componente puro `Sopro.Calculation.Financing.FinancingPreparationCalculator`. Fase 1 (`PrepareBaseSchedule`): acumula el CD distribuido multiplicando cantidad × precio unitario visible (redondeo de importe del proyecto) y absorbe el residuo (esperado − distribuido) en el último período con monto; acumula la estimación cobrable desde `ImporteProgramado` redondeando tras cada suma — el residuo NO contamina la estimación. Fase 2 (`ReconcileIndirectCost`): replica la reconciliación del legado (reparto proporcional sobre filas con monto, o sobre la base de CD cuando el actual es cero, con el total oficial en la última fila cuando la base también es cero) y fija egreso = CD + CI redondeado. El **total de indirectos oficiales** se sigue calculando en Application con la única implementación del preview de referencia del presupuesto (sin duplicarse en el paquete); el adaptador conserva consultas EF, filtro/agrupación/orden, mapeo y persistencia, y eliminó la construcción del `engine` y la rama muerta de desfase. Contrato inmutable real (`ReadOnlyCollection`), validación fail-fast de índices en pasada inicial y nulls explícitos. 24 tests puros + test de mapeo multi-concepto; goldens 8/8 y Debug/Release 611/611; baseline de API regenerado.
